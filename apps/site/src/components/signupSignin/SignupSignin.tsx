...
const SignUpForm = dynamic(() => import("../loginRegistrationForm/SignUpForm"), {
  ssr: false,
});
...
const SignupSignin = ({ setLoginModalOn, LoginmodalOn }: Props) => {
  ...
  return (
    <div>
      <div
        className={`fixed w-full h-full left-0 top-0 z-20 bg-[#18181B] opacity-50  ${
          LoginmodalOn ? "opacity-40 visible" : "invisible scale-0 opacity-0"
        }`}
      />
      <div
        className={`fixed w-full h-full left-0 top-0 z-50 flex items-center justify-center transition-all ease-in duration-300 ${
          LoginmodalOn ? "scale-100 visible opacity-100" : "invisible scale-0 opacity-0"
        }`}
      >
        <div className="p-5 md:p-10 rounded-2xl w-[340px] sm:w-[440px]  xl:w-[480px] z-50 bg-white relative transition-all ease-in-out duration-300">
          <div>
            ...
            <div className=" flex items-center justify-center gap-20 relative">
              ...
              <div
                className={`border border-themePrimary600 ] mt-4 absolute -bottom-0.5 w-[140px]  ${
                  active === 0
            ...
            {(active === 0 && <LoginForm setLoginModalOn={setLoginModalOn} />) ||
              (active === 1 && <SignUpForm setActive={setActive} />)}
          </div>
        </div>
        <div
          className={`fixed w-full h-full left-0 top-0 z-20 transition-all ease-in-out duration-300 ${
            LoginmodalOn ? "block" : "hidden"
          }`}
          onClick={() => setLoginModalOn(false)}
        />
      </div>
